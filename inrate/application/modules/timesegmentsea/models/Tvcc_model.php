...
ORDER BY `DATE`,M1
	...
	$name_tb = strtoupper(date_format(date_create($arr_dates),"yM"));  
		$db = $this->clickhouse->db();
			...
			$sql = "
				SELECT *,CONCAT(PROGRAM,'||',formatDateTime(SPLIT_MINUTES,'%Y-%m-%d'),' ',START_TIME) AS FULL_PROG, formatDateTime(SPLIT_MINUTES,'%H:%M') AS MINUTES FROM `RATING_PER_MINUTES_".$name_tb."` A
