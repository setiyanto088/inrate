...
class Tvprogramun_model extends CI_Model {
	...
	public function get_epg_file($param){
		...
		$query = " SELECT * FROM FILE_UPLOAD_EPG_DEV WHERE ID_USER = '".$param['iduser']."' AND TOKEN = '".$param['token']."' ";
		//echo $query;die;
	...
	public function get_list_channel($date){
		...
		WHERE A.`STATUS` = 1
			...
			SELECT CHANNEL,COUNT(*) SD, DATE_FORMAT(START_TIME,'%Y-%d-%d') AS DTS FROM (SELECT * FROM EPG_RAW1_TEST_DEV GROUP BY `CHANNEL`,`PROGRAM`,`START_TIME`) ec 
	...
	public function save_file_channel($params){ 
		...
		$sql 	= "INSERT INTO FILE_UPLOAD_EPG_DEV(`FILENAME`,`CHANNEL`,`START_TIME`,END_TIME,UPLOAD_DATE,TOKEN,ID_USER,TOTAL_ROW) 
