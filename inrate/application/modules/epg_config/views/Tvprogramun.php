...
$(document).ready(function(){
	...
	$("#drop_zone").on("drop", function(event) {
		...
		for(var f = 0; f < fileobj.length; f++) {
			var tokent = makeid(10);
			
			var fname = fileobj[f].name;
			var fsize = fileobj[f].size;
			if (fname.length > 0) {
				document.getElementById('file_info').innerHTML += "File name : " + fname +' (<b>' + bytesToSize(fsize) + '</b>) <input type="checkbox" class="messageCheckbox" name="fileup" value="'+tokent+'" checked > <br>';
			}
...
function processData(rdn_ar) {
	...
	var urls = '<?php echo base_url(); ?>epg_config'; 
	
	var checkedValue = document.querySelectorAll('input[name=fileup]:checked');
	var token_proc = '';
	var selectedCboxes = Array.prototype.slice.call(checkedValue).filter(ch => ch.checked==true);
	for(var f = 0; f < selectedCboxes.length; f++) {
		
		token_proc += selectedCboxes[f].value +'|';
	}
	
	form_data.append('token_proc', token_proc);
...
if(file_obj != undefined) {
	...
	$.ajax({
		...
		success:function(response) {
			$('#message_info').html(response);
			if(response == 'Session Expired, Please Login Again'){
				window.location = urls;
			}
			//alert(response);
			$('#selectfile').val('');
