...
class Audience_model extends CI_Model {
  ...
  public function channel_set($preset,$userid){ 
	 ...
	 if($sdate == $enddate){
		  $query2 = "SELECT PROGRAM,START_PROGRAM FROM `M_SUMMARY_AUDIENCE_N_PTV`
  			WHERE `DATE` BETWEEN '".$sdate."' AND '".$enddate."' 
  			AND CHANNEL = '".$scha."' AND PROGRAM LIKE '%".$sprog."%'
			GROUP BY PROGRAM,START_PROGRAM
  			ORDER BY program";
	 }ELSE{
        $query2 = "SELECT PROGRAM FROM `M_SUMMARY_AUDIENCE_N_PTV`
  			WHERE `DATE` BETWEEN '".$sdate."' AND '".$enddate."' 
  			AND CHANNEL = '".$scha."' AND PROGRAM LIKE '%".$sprog."%'
			GROUP BY PROGRAM
  			ORDER BY program";
	...
	public function tree1(){
		...
		$query = "
		SELECT A.*,B.`FIELD` AS PARENTFIELD, B.DEFAULT AS PARENTDEFAULT FROM `TREE_PROFILING_RES_P22` 
		A LEFT JOIN `TREE_PROFILING_RES_P22` B
		ON A.`PARENTID` = B.`ID`
	...
	public function channelsearch($key,$kkk){       
        ...
        // ORDER BY C.`CHANNEL`";
		 ...
		 $sql = "SELECT CHANNEL_NAME_PROG AS CHANNEL_CIM, CHANNEL_NAME_PROG AS CHANNEL_CDR FROM `CHANNEL_PARAM_FINAL` C
		 WHERE FLAG_TV = 1 AND UPPER(CHANNEL_NAME_PROG) LIKE '%".strtoupper($key)."%'
		 GROUP BY CHANNEL_NAME_PROG
        ORDER BY C.`CHANNEL_NAME_PROG`";
