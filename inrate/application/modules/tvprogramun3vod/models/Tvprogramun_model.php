...
SELECT CHANNEL AS channel,VIEWERS AS Spot FROM M_SUM_TV_DASH_CHAN_DURATION_WEEK_PTV R WHERE 
	...
	public function list_spot_by_program_all2Ps_new($field,$wheres,$params,$pilihprog,$profile,$query_qr2) {
			...
			if($pilihprog == 'AUDIENCE'){
				$tbt = 'M_SUM_TV_DASH_PROG_PTV';
			}ELSEif($pilihprog == 'TOTAL_VIEWS'){
	...
	public function list_spot_by_program_all2Ps_new_export($field,$wheres,$params,$pilihprog,$profile,$query_qr2) {
		...
		$where = " AND (PROGRAM LIKE '%".$params['searchtxt']."%' OR CHANNEL LIKE '%".$params['searchtxt']."%') ";
			...
			SELECT COUNT(*) AS jumlah FROM 
				( 
			SELECT DISTINCT a.`'.$field.'`,CHANNEL, VIEWERS AS Spot FROM '.$tbt.' a
						WHERE 1=1 
						AND ID_PROFILE = "'.$profile.'" '.$wh_chn.' 
						AND TANGGAL="'.date('Y').'"  '.$where.'
						#GROUP BY CHANNEL,a.`'.$field.'`
						ORDER BY Spot DESC
						)z 
						';
			...
			SELECT COUNT(*) AS jumlah FROM 
				( 
			SELECT DISTINCT a.`'.$field.'`,CHANNEL, SUM(VIEWERS) AS Spot FROM '.$tbt.' a
						WHERE 1=1 
						AND ID_PROFILE = "'.$profile.'" '.$wh_chn.' 
						AND SUBSTR(`TANGGAL`,1,4)="'.date('Y').'"  '.$where.'
						GROUP BY CHANNEL,a.`'.$field.'`
						ORDER BY Spot DESC
						)z 
						';
