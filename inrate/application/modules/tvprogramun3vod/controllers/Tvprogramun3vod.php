		   ...
		   $it1 = 2;
			...
			foreach($data['monthdt'] as $wkwk){
				...
				SELECT z.*, rank() over ( ORDER BY VIEWERS DESC,CHANNEL DESC) AS Rangking FROM 
												( 
							SELECT 
							SUBSTRING_INDEX(CONTENT1, '|', 1) AS CHANNEL,
							SUBSTRING_INDEX(CONTENT1, '|', -1) AS PROGRAM,`VALUES` AS VIEWERS FROM `SUMMARY_VOD`
							WHERE `CONTENT3` = ".($wkk)."
			...
			A1.VIEWERS AS TOTAL 
					FROM (
					SELECT z.*, rank() over ( ORDER BY VIEWERS DESC,CHANNEL,PROGRAM) AS Rangking FROM (
						SELECT SUBSTRING_INDEX(CONTENT1, '|', 1) AS CHANNEL,
						SUBSTRING_INDEX(CONTENT1, '|', -1) AS PROGRAM
		...
		if($this_day == "All"){
				...
				IF($type == 'AUDIENCE'){
					...
					$tbt = 'M_SUM_TV_DASH_CHAN_VIEWERS_PTV';
						$query_qr2 = $query_qr2.''.$week_in2.' FROM (
									...
									AND STR_TO_DATE(PERIODE,"%Y-%M") < STR_TO_DATE("'.date('Y-F').'","%Y-%M")
									AND ID_PROFILE = 0
									'.$where.'
									GROUP BY CONTENT1
								) CHN '.$join_left2.' WHERE 1=1  ORDER BY CHN.VIEWERS DESC ';
					...
					IF($ri == 1){
						...
						$join_left = $join_left." 
							LEFT JOIN (	
								SELECT z.*, rank() over ( ORDER BY VIEWERS DESC,CHANNEL DESC) AS Rangking FROM 
									( 
										SELECT CONTENT1 AS `CHANNEL`,`VALUES` AS VIEWERS,PERIODE AS `TANGGAL`,CONTENT3 AS `WEEK`,`ID_PROFILE` FROM SUMMARY_VOD
			...
			$ri_last = $ri - 2;
				...
				$query_qr = $query_qr." A1.VIEWERS AS TOTAL
					FROM (
						SELECT rank() over ( ORDER BY CONTENT1 DESC) AS Rangking,
						CONTENT1 AS CHANNEL FROM SUMMARY_VOD
